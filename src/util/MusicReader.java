    ...
    while (scanner.hasNext()) {
      ...
      switch (lineType) {
        ...
        case "note":
          ...
          case "startrep":
              try {
                  int startRep = scanner.nextInt();
                    Sign temp = new Sign(startRep, 1);
                  piece.addStartRep(temp);
              }
              catch (NoSuchElementException e) {
                  throw new IllegalArgumentException("Malformed note line: " + scanner.nextLine());
              }
          ...
          case "contrep":
              try {
                  int contRep = scanner.nextInt();
                  Sign temp = new Sign(contRep, 0);
                  piece.addContRep(temp);
              }
              catch(NoSuchElementException e) {
                  throw new IllegalArgumentException("Malformed note line: " + scanner.nextLine());
              }
          ...
          case "specrep":
              try {
                  int specRep = scanner.nextInt();
                  Sign temp = new Sign(specRep, -1);
                  piece.addSpecRep(temp);
              }
              catch (NoSuchElementException e) {
                  throw new IllegalArgumentException("Malformed note line: " + scanner.nextLine());
              }
